...
class DataBase{
	...
	static int callbackexport(void* outputStruct, int countRec,char** argv,char** colName);
	bool exportbd(const char* sqlString,FILE* f);
        bool SelectidFromArsize(int arsize, int &id);
	bool selectlist(const char* sqlString);
};
