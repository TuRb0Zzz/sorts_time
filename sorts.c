...
void checkkeys(int argc, char **argv, int* flags, int& maxsize, int& step,int& list,int& exportbd){
	    bool changesteps=0;
            if (argc > 1) {
			    ...
			    else if(std::string(argv[i])=="--export"){
				    exportbd=1;
			    }
            ...
            if (maxsize!=step and changesteps==0){
            	step = maxsize;
            }
            }
            else{
	...
	int DataBase::callback(void* outputStruct, int countRec, char** argv, char** colName) {
            //void* -- c-style
	...
	int DataBase::callbackexport(void* outputStruct, int countRec, char** argv, char** colName) {
	    FILE* tempfile = (FILE*)outputStruct;
	    tempfile  = fopen("DataBase.txt","a");
            for (int i = 0; i < countRec; i=i+4) {
		fprintf(tempfile,"%s \t %s \t %s \t %s \t %s \t %s \t %s \t %s\n",colName[i],argv[i],colName[i+1],argv[i+1],colName[i+2],argv[i+2],colName[i+3],argv[i+3]);
            }
	    fclose(tempfile);
            return 0;
        }
        bool DataBase::selectlist(const char* sqlString) {
            char* errMsg;
	...
	bool DataBase::exportbd(const char* sqlString,FILE* f) {
            char* errMsg;
            int er = sqlite3_exec(bd, sqlString, callbackexport, f, &errMsg);
            if (er != SQLITE_OK) {
                std::cerr << "error request " << sqlString << " : " << errMsg << std::endl;
            }
            return er;
